...
function clearForm() {
  ...
  document.getElementById('corError').textContent = '';
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function validateCor() {
    ...
    const error = document.getElementById('corError');
    const value = corInput.value.trim();

    if (!/^#[0-9A-Fa-f]{6}$/.test(value)) {
      error.textContent = 'Cor hexadecimal inválida.';
      corInput.classList.add('input-error');
      return false;
    }

    error.textContent = '';
    corInput.classList.remove('input-error');
    return true;
  }
  ...
  function validateAllFields() {
    return validateTitulo() && validateCor() && validateInicio();
  }
  ...
  function editCampanha(id) {
    fetch(`/api/campanhas/${id}`)
      ...
      .then(campanha => {
        ...
        document.getElementById('campanhaTitulo').value = campanha.titulo;
        document.getElementById('campanhaCor').value = campanha.cor;
        document.getElementById('campanhaInicio').value = campanha.inicio || '';
        document.getElementById('campanhaFim').value = campanha.fim || '';
    ...
    fetch('/api/campanhas')
      ...
      .then(campanhas => {
        ...
        campanhas.forEach(campanha => {
          ...
          const is_checked = campanha.status === 'Ativo' ? 'checked' : '';
